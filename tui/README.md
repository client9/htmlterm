...
Ctrl-X cuts from the focused field via `Document.DispatchCut`, handing the returned text to `Screen.SetClipboard` — but only on a terminal that reports clipboard support (`Screen.HasClipboard`, i.e. OSC 52). That check runs *before* the dispatch, since `DispatchCut` removes the text as part of dispatching: cutting into a nonexistent clipboard would destroy it outright, with no undo, and with a collapsed caret a cut takes the whole field rather than a selection. Shift+Tab (tcell's `KeyBacktab`) is normalized to `"Tab"` plus a Shift modifier so `DispatchKey` walks the tab order backwards.
