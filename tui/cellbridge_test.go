...
// sequences (terminal hyperlinks), the same way htmlterm_test.go's own
// stripANSI does for its table-driven tests — duplicated here rather than
// exported from htmlterm since it's a test-only comparison helper, not
// something a real consumer of the package needs.
...
func TestPaintLinesMatchesRenderedOutput(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			r, err := htmlterm.New(htmlterm.Options{CSS: tc.css, Width: tc.width})
			if err != nil {
				t.Fatalf("New: %v", err)
			}
