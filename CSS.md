...
| `hr` | Horizontal rule. The UA stylesheet provides `hr { display: block; border-top: "─"; }`, drawing a full-width line. It is an ordinary bordered block box with only its top edge set, so it takes the same [`border-top`](#border-left-border-right-border-top-border-bottom) property any other element does: a quoted glyph (`hr { border-top: "="; }`) to change the character, which may be more than one character and is tiled to the box's exact width (`hr { border-top: "=-"; }`); a preset name (`hr { border-top: double; }`) for a themed line; and `border-top-color`, or a color folded into the same declaration (`border-top: double red`), to change its color. Use the per-edge form, not the whole-box [`border-style`](#border-style) property: `border-style` on `hr` still keeps the rule to one line, but since the UA stylesheet already claims `border-top` for the default character, a preset's own top glyph never shows through, only its corners do (`hr { border-style: double; }` draws `═` corners around an otherwise plain `─` line rather than a `═` rule). |
...
| `fieldset` | Groups related form controls in a bordered box (default: `display: block; border-style: solid; padding: 1; margin-bottom: 1`). On a live `Document`, a `disabled` attribute disables every form control nested inside it (click, focus, tab order, typing), except those nested in its own first `<legend>` child, matching HTML's fieldset-disabling algorithm exactly, including for nested fieldsets: an inner fieldset's own legend exempts only that fieldset's disabling, not an outer disabled fieldset's. `:disabled` matches a cascaded-disabled control the same way it already does for an `<option>` disabled via its `<optgroup>`. |
...
| `datalist` | Suggestion source for an `<input list>`; never rendered itself (default: `display: none`). On a live `Document`, typing in the associated input opens a popup of its `<option>`s filtered by a case-insensitive prefix match on `value`; ArrowDown opens it, arrow keys browse, Enter or a click picks one (firing `"input"` then `"change"`), Escape dismisses it. The `<datalist>`'s own declarations style the popup, and `option:hover` styles the highlighted row, the same pseudo-class `<select>`'s popup uses. **See `docs/DATALIST.md`** for the full styling reference and keybindings. |
