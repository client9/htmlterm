...
htmlterm treats document-provided HTML and CSS text as untrusted terminal content
...
`Document.Render()` or `Renderer.Render()` call, meaning content this package
already rendered, and in doing so already sanitized, once, rather than
document-provided text.

This exists for hosts that re-render the same content on every repaint, such
as scroll-triggered redraws, and want to pay layout cost once: render a
subtree to a string, cache it, and re-embed the cached string on subsequent
repaints instead of re-parsing and re-laying-out the original HTML each time.
Embedding a raw ANSI string via `SetInnerHTML` does *not* work. Its sanitizer
strips the escape sequences that carry the styling, silently degrading the
output to unstyled text. That is why `SetPreRendered` exists as a distinct,
explicit call instead.

The mechanism is this. `SetPreRendered` attaches `ansi` as a single
[`html.RawNode`](https://pkg.go.dev/golang.org/x/net/html#NodeType) child of
`el`. `html.Parse` and `html.ParseFragment`, and therefore `SetInnerHTML`,
which is built on `ParseFragment`, never produce a `RawNode` from any input:
...
node in the tree*, not by any property of the string itself or by a naming or
wrapper convention a caller could accidentally defeat. The only way a
`RawNode` enters a `Document`'s tree is a direct `SetPreRendered` call.

That said, `SetPreRendered` performs **no validation** of its own. It trusts
the caller completely. It does not check that `ansi` is well-formed ANSI, that
it was produced by this package, or that its line widths match `el`'s resolved
content width; a caller re-using stale output after a resize will see it
clipped or padded to the new width, not rewrapped. Passing it arbitrary or
attacker-controlled text reintroduces exactly the terminal-escape-injection
risk the rest of this document describes. `SetPreRendered` is an escape hatch
for content this package already rendered, not a general-purpose "trust me"
API. Callers should treat it with the same care as Go's
`html/template.HTML`: a real tool for a real problem, and a real risk if fed
the wrong input.

## Untrusted HTML and CSS

htmlterm may be used with untrusted HTML and CSS for terminal rendering, but callers
...
- limiting input size before parsing or rendering
...
- crashes or excessive resource use from crafted HTML or CSS
