...
// defaultDialogModalAttr is the reserved marker attribute (see
// defaultFocusAttr) a <dialog> carries while it is open as a modal, meaning
// opened by ShowModal rather than Show. It is set and cleared by document's
// Element.ShowModal and closeDialog.
//
// Layout never reads it. It reaches rendering through the cascade instead, as
// cssengine.Cascade.ModalAttr, which is what makes the UA stylesheet's
// `dialog:modal` rule match and hand the element to the ordinary
// position:fixed out-of-flow path, with no compositing code of its own. The
// one place that does check it directly is paintBackdrop (outofflow.go),
// since ::backdrop applies to a top-layer element and nothing else. See
// docs/DIALOG.md.
