...
// in *cells*, the unit a terminal charges for, rather than in bytes or runes.
//
...
//   - cells, or columns: what a terminal charges. What Width, VisibleLen,
...
// the two apart. The repository's column-invariant suite, in
...
// DOM's selectionStart, selectionEnd, and setSelectionRange are offsets into
// the value's character sequence, and document mirrors them so a caller that
...
// are called at the edges, when placing a terminal cursor or turning a click
// into an offset. Everything on the document side of that boundary stays in
// runes; everything on the painting side stays in cells. "These are all just
// character counts, unify them" is the tempting wrong move, and it would
// silently break every non-ASCII selection.
package textcell
...
// widthOptions mirrors tcell/v3's internal/widthutil.Options() byte for
// byte. tcell derives its own displaywidth.Options from the
...
// find one grapheme cluster's boundary. Real-world multi-rune clusters, such
// as ZWJ emoji sequences, skin-tone modifiers, regional-indicator flag pairs,
// and VS16 pairs, are well under this. If a pathological input has no cluster
...
// pairs, ZWJ sequences, regional-indicator flag pairs, skin-tone modifiers.
// displaywidth's grapheme segmenter finds the boundary and scores the whole
// cluster as one unit, so callers no longer need to track a "previous rune's
// width" the way the old VS16-only correction did.
//
...
// from the frame this package's own layout and wrapping already measured,
// which is what previously caused a wide emoji to shift every subsequent
...
// Width is the terminal column width of an entire plain, escape-free string:
// the sum of each grapheme cluster's display width, with VS16 pairs, ZWJ
// sequences, flag pairs, and the rest already scored as one unit. There is no
...
// s. Rune offsets and columns are not interchangeable, since a CJK or emoji
...
//
// Exported for htmlterm/tui's use, in focusCursorPos, for the same reason
// NextGrapheme is: the terminal's own hardware cursor has to land on the
// column this package's layout put that rune at.
...
// cluster's own offset rather than the following one, so clicking either cell
// of a wide character puts the caret before it, and clicking a cell then
// drawing the cursor for the resulting caret lands back on the same cluster.
// A column past the end of s returns len([]rune(s)).
//
// Exported for document's use, in caretIndexFromClick, the click-to-caret
...
// Carry tracks the single currently-open SGR style span, the single
// currently-open OSC8 hyperlink span, or both, while scanning
// already-styled or hyperlinked text left to right. A line break inserted
// mid-span, by word-wrap or a hard break, can then close the span before the
// break and reopen the identical span right after it, making every emitted
// line independently self-contained.
//
// It is deliberately flat rather than a stack. htmlterm's styling pipeline,
// style.go's inlineStyle.render plus block.go's wrapHyperlink, only ever
// emits one combined SGR open and one full reset per span, and one OSC8
// open and one OSC8 reset per hyperlink. There are never partial resets and
// never two concurrently-open spans of the same kind. If that invariant
// changes, this needs to become a stack.
type Carry struct {
...
// empty or "0" params clears the active SGR span, and any other SGR sequence
// replaces it. An OSC8 sequence with an empty URI clears the active
// hyperlink span, and any other OSC8 sequence replaces it. Anything else is
// left untouched.
...
// reset, meaning empty or "0" params, as opposed to a style-setting sequence.
...
// hyperlink, meaning an empty URI, as opposed to opening one.
...
// CollapseDeadSpans removes SGR and OSC8 "open" escape sequences that are
// immediately superseded by a full reset of the same kind, with no visible
// character in between: spans that never got the chance to style or link
// anything. Dropping such an open and keeping only the reset is always safe
// regardless of any earlier context. A full SGR reset, "\x1b[m" or "\x1b[0m",
// or an OSC8 reset, the empty-URI "\x1b]8;;...", unconditionally clears that
// kind of state no matter what, if anything, was open beforehand, so whether
// the dead open ran first makes no observable difference.
//
...
// supersedes A. This codebase's SGR sequences are additive rather than
// full-replace (see Carry's doc comment), so "\x1b[3;4m", italic plus
...
// word's token instead, and the line-wrap carry mechanism, ensureOpen and
// closeAndPush, can then reopen a span right before that misplaced close
// cancels it out again. One case is an <a>'s own trailing single-space text
// node, styled only with underline, sitting right at a word-wrap boundary
// next to a differently-styled neighbor. The result is well-formed but
// pointless escape sequences like "\x1b[3m\x1b[m" wrapping zero characters.
// This is a final cleanup pass, not a fix for the token misattribution
// itself. It only guarantees the misattribution can never surface as a
// dangling or empty span in output.
func CollapseDeadSpans(s string) string {
	...
	// pendingSGROpen and pendingOSCOpen hold the out-index where a
	// not-yet-superseded SGR-open or OSC8-open sequence respectively starts,
	// as long as no visible character has appeared since, meaning dropping it
	// is still safe if the next same-kind sequence turns out to be a full
	// reset. -1 means there's no such pending, droppable open.
...
// SplitAtWidth is splitAtVisualWidth's carry-aware core. start is whatever
// SGR or OSC8 span is already open as of the start of s, from preceding text
// this call doesn't see, and the returned carry is whatever span is left open
...
// VisibleWindow returns the [offset, offset+width) visible-column window of
// s. It is wide-rune and ANSI aware, and, like SplitAtWidth's internal chunk
// boundaries, self-contained: an SGR or OSC8 span still open at offset is
// reopened at the window's start and closed at its end. It is used for
...
// SplitAtWidth, which only ever chops a string into sequential chunks from
// column 0. A wide rune straddling either boundary is dropped entirely rather
// than split, the same rule VisiblePrefix already applies at its own single
// right-hand boundary.
...
// replacement, preserving line's ANSI styling for the untouched prefix and
// suffix and re-carrying any open span across the splice boundary. It is the
// primitive popup and overlay rendering needs (see docs/RENDERING.md's popup
...
// splitAtVisualWidth and SplitAtWidth, which only chop a string into
// sequential chunks from column 0 and have no notion of an interior
// overwrite. replacement is inserted verbatim, and the caller is responsible
// for it being exactly width visible columns wide, a popup's own
// already-rendered box.width for instance, so the result's total visible
// width matches line's. SpliceColumns does not itself pad or truncate it.
//
...
func SpliceColumns(line string, col, width int, replacement string) string {
	...
	// was active there. That is the same span, if any, that covered the
	// entire cut region and would otherwise be orphaned, with its opening
	// sequence discarded and only its eventual reset surviving in the
	// suffix.
	skipEnd := col + width
...
// TrimTrailingSpace removes s's last visible, non-ANSI-escape rune if it's a
// space, preserving every escape sequence exactly where it was, including
// ones immediately before or after the removed rune, so a styled span's
// opening and closing codes stay correctly paired around the shortened
// content. ok is false, and s is returned unchanged, if s has no visible
// content or its last visible rune isn't a space.
...
func VisiblePrefix(s string, width int) string {
	...
	for i < len(runes) && visible < width {
		...
		// the boundary must not be included. Including it overshoots width
		// by 1 or more, which is what produced the scrollbar-gutter
...
// runes[i], which must be '\x1b'. It is the same recognizer every ANSI-aware
// helper in this package tokenizes escape sequences with, including
// wordWrapTokens, VisibleLen, and the Carry state machine. It delegates to
// x/ansi's decoder rather than hand-rolling CSI and OSC recognition a second
// time. runes[i:] is re-encoded to a string because the decoder works in
// bytes, and the consumed byte count is converted back to a rune count since
// every caller here indexes into a []rune.
...
// costs two. suffix is appended when content is clipped — use "" for a clip
// with no indicator, "…" for an ellipsis — and is itself measured the same
// way.
